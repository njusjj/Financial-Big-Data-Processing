package RandomClusterGenerator;
    ...
    public void makeDecision(Instance instance){
        if(kClusters.size() < k){
            ...
            if(!(choice == -1)){
                ...
                kClusters.remove(choice);                    //将之前的中心去掉
                Cluster cluster = new Cluster(id, instance);
                kClusters.add(cluster);                        //替换添加新的中心
            }
        }
    }

    /**
     * 以1/(1+k)的概率，返回一个[0,k-1]的正整数，以       意思是在后面读入初始点时也依概率挑选出点，
     * k/k+1的概率返回-1.                              将原来的初始的作为聚类中心的点去掉，然后替换，保证不会总是前三个。
     * @param k
     * @return
     */
